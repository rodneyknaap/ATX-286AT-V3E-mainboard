...
The design functions well with a 64MHz oscillator and also is able to function with a 32MHz oscillator where we get a slower 8MHz CPU processing clock speed. However operating the CPU at 8MHz with this release does have some minor side effects of the UART port detection by MR BIOS sometimes resulting in adding a few "ghost" COM ports to the system. This can be seen by "hardware changed" beeping of MR BIOS prompting to save the changes after which you can boot normally and use the system at 8MHz CPU speed. However in any case, after saving with F10 in the BIOS, the COM1 port always remains functional and otherwise the release functions stable at 8MHz as well. Generally the release is intended for using a 64MHz oscillator so operation may differ when using other frequencies which builders can try for themselves.  
