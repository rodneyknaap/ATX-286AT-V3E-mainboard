...
This first release features a more synchronous system control. The design has been verified with a 64MHz clock oscillator for FCLOCK, internally in the quartus clock the signal is named 2FCLOCK to indicate the double clock speed. This is then divided by two to provide the FCLOCK frequency. In this fixed clock release, 286_CLK going to the 286 CPU is always of equal frequency with FCLOCK.  
The design functions well with a 64MHz oscillator. In addition it is able to function with a 32MHz oscillator as well, where we get a slower 8MHz CPU processing clock speed. However using a slower 32MHz oscillator and operating the CPU at 8MHz with this release does have some minor side effects where the UART port detection by MR BIOS sometimes results in adding a few "ghost" COM ports to the system. This can be seen by "hardware changed" beeping of MR BIOS prompting to save the changes after which you can boot normally and use the system at 8MHz CPU speed. However in any case, after saving with F10 in the BIOS, the COM1 port has always been functional in my tests and otherwise the release functions stable at 8MHz as well if you would want to test this. So as the file name indicates, primarily this release is intended for using a 64MHz oscillator with which it operates best, and operation may differ when using other frequencies which builders can try for themselves, where 32MHz has shown to be a more favorable oscillator frequency in the test results compared to some of the other frequencies tested.  
